...
func execDotGeneralSmallMatMulBFloat16(_ *Backend, lhs, rhs *Buffer, params *dotGeneralNodeData, output *Buffer) { //alt:bf16
		...
		for m := range lhsCrossSize {
			...
			for n := range rhsCrossSize {
				...
				for ; k+3 < contractingSize; k += 4 {
					...
					sum += lhsFlat[lhsRowStart+k].Float32()*rhsFlat[rhsColStart+k*rhsColStride].Float32() + //alt:bf16
						lhsFlat[lhsRowStart+k+1].Float32()*rhsFlat[rhsColStart+(k+1)*rhsColStride].Float32() + //alt:bf16
						lhsFlat[lhsRowStart+k+2].Float32()*rhsFlat[rhsColStart+(k+2)*rhsColStride].Float32() + //alt:bf16
						lhsFlat[lhsRowStart+k+3].Float32()*rhsFlat[rhsColStart+(k+3)*rhsColStride].Float32() //alt:bf16
					//alt:f16 sum += lhsFlat[lhsRowStart+k].Float32()*rhsFlat[rhsColStart+k*rhsColStride].Float32() +
					//alt:f16 lhsFlat[lhsRowStart+k+1].Float32()*rhsFlat[rhsColStart+(k+1)*rhsColStride].Float32() +
					//alt:f16 lhsFlat[lhsRowStart+k+2].Float32()*rhsFlat[rhsColStart+(k+2)*rhsColStride].Float32() +
					//alt:f16 lhsFlat[lhsRowStart+k+3].Float32()*rhsFlat[rhsColStart+(k+3)*rhsColStride].Float32()
				}
