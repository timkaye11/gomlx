...
func (b *Builder) getOrCreateBlockedInput(rhs *Node) *Node {
	// Check the legacy blockedForDotGeneral map for backward compatibility
	if b.blockedForDotGeneral != nil {
	...
	// Also populate the legacy map for backward compatibility with tests
	if b.blockedForDotGeneral == nil {
		b.blockedForDotGeneral = make(map[*Node]*Node)
	}
