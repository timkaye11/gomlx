...
type nodeDedupKey struct {
	...
	inputCount int
	firstInput uintptr // unsafe.Pointer of first input node, or 0 if no inputs
}
...
func makeNodeDedupKey(opType backends.OpType, inputs []*Node) nodeDedupKey {
	...
	if len(inputs) > 0 {
		key.firstInput = uintptr(unsafe.Pointer(inputs[0]))
	}
	return key
}
...
type Builder struct {
	...
	// Deprecated: use nodeDedup instead. Will be removed after migration.
